...
void creatUDPConnection()
{
    ...
    cout << "The serverEE is up and running using UDP on port " << PORT << "." << endl;
}
...
void checkMessage()
{
    ...
    if(cat != ALL){
        cout << "The serverEE received a request from the Main Server about the " << getStringFromCategory(cat) << " of " << code << "." << endl;
    }
